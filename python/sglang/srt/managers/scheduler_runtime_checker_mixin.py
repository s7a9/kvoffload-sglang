...
class SchedulerRuntimeCheckerMixin:
    def check_kv_offload_policy_decision(
        ...
        resume_ids = [req.rid for req in decision.new_resume_list]
        ...
        if len(resume_ids) != len(set(resume_ids)):
            logger.warning("KV offload policy produced duplicate new_resume_list.")
            return False
        ...
        if not set(keep_running_ids).issubset(running_ids):
            ...
            return False
        if not set(resume_ids).issubset(waiting_ids):
            logger.warning(
                "KV offload policy new_resume_list is not a subset of waiting reqs."
            )
            return False
