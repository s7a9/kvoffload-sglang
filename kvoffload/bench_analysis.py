...
def build_token_arrival_df(rec: Dict) -> pd.DataFrame:
    ...
    itls = rec.get("itls", [])
    output_lens = rec.get("output_lens", [])
    send_times = rec.get("send_times", [])  # Time when request was sent (relative to benchmark start)
    ...
    for req_id in range(n):
        ...
        # Get send time if available, otherwise use 0
        send_time = float(send_times[req_id]) if send_times and req_id < len(send_times) else 0.0
        ...
        if token_count == 1:
            # Absolute time: send_time + ttft
            abs_times = np.array([send_time + ttft], dtype=float)
        else:
            partial_itls = np.array(itl_list[: token_count - 1], dtype=float)
            # Absolute times: send_time + ttft for first token, then add cumulative ITLs
            abs_times = np.concatenate(([send_time + ttft], send_time + ttft + np.cumsum(partial_itls)))
...
def plot_grouped_timelines(experiments_by_setting: Dict[str, List[Dict]], plot_dir: Path):
    """Plot all methods for each setting in a single figure."""

    for setting, experiments in experiments_by_setting.items():
        if not experiments:
            continue

        # Sort experiments by method order: default, local-speed15, local-speed20, local-speed25
        method_order = ["default", "local-speed15", "local-speed20", "local-speed25"]
        experiments.sort(key=lambda x: method_order.index(x["method"]) if x["method"] in method_order else 999)

        n_methods = len(experiments)

        # Create figure with subplots (one row per method)
        fig_abs, axes_abs = plt.subplots(n_methods, 1, figsize=(14, 4 * n_methods))
        fig_rel, axes_rel = plt.subplots(n_methods, 1, figsize=(14, 4 * n_methods))

        if n_methods == 1:
            axes_abs = [axes_abs]
            axes_rel = [axes_rel]

        fig_abs.suptitle(f"Token Arrival Timeline (Absolute) | {setting}", fontsize=16, fontweight="bold")
        fig_rel.suptitle(f"Token Arrival Timeline (First token = 0) | {setting}", fontsize=16, fontweight="bold")
        ...
        for idx, exp in enumerate(experiments):
            ...
            for _, g in df_tokens.groupby("request_id"):
                ax_abs.plot(g["abs_time_s"].to_numpy(), g["token_idx"].to_numpy(), alpha=0.25, linewidth=1.0)
            ax_abs.set_title(method_label, fontsize=12, fontweight="bold")
            ax_abs.set_ylabel("Token Index", fontsize=10)
            ax_abs.grid(alpha=0.25)
            if idx == n_methods - 1:
                ax_abs.set_xlabel("Time (s)", fontsize=10)
            else:
                ax_abs.set_xlabel("")

            # Plot relative timeline
            ax_rel = axes_rel[idx]
            for _, g in df_tokens.groupby("request_id"):
                ax_rel.plot(g["rel_time_s"].to_numpy(), g["token_idx"].to_numpy(), alpha=0.25, linewidth=1.0)
            ax_rel.set_title(method_label, fontsize=12, fontweight="bold")
            ax_rel.set_ylabel("Token Index", fontsize=10)
            ax_rel.grid(alpha=0.25)
            if idx == n_methods - 1:
                ax_rel.set_xlabel("Time Since First Token (s)", fontsize=10)
            else:
                ax_rel.set_xlabel("")

        # Save figures
        safe_setting = _safe_name(setting)
        abs_path = plot_dir / f"{safe_setting}_token_arrival_abs.png"
        rel_path = plot_dir / f"{safe_setting}_token_arrival_rel0.png"

        fig_abs.tight_layout()
        fig_abs.savefig(abs_path, dpi=150)
        plt.close(fig_abs)

        fig_rel.tight_layout()
        fig_rel.savefig(rel_path, dpi=150)
        plt.close(fig_rel)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Read bench JSONL, export metrics CSV, and generate grouped timeline plots."
    )
    parser.add_argument(
        "--jsonl-path",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="If non-empty, save grouped timeline plots into this directory.",
    )
    parser.add_argument(
        "--tag-glob",
        type=str,
        default="",
        help="If non-empty, only include experiments whose tag matches this glob pattern.",
    )
    parser.add_argument(
        "--list-tags",
        action="store_true",
        help="List tags in the JSONL and exit without exporting CSV/plots.",
    )
