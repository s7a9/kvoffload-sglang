...
KV_OFFLOAD_ENABLE_EMERGENCY_EVICTION="${KV_OFFLOAD_ENABLE_EMERGENCY_EVICTION:-1}"
KV_OFFLOAD_EMERGENCY_MIN_EVICT_TOKENS="${KV_OFFLOAD_EMERGENCY_MIN_EVICT_TOKENS:-256}"
KV_OFFLOAD_EMERGENCY_PREFILL_RETRY="${KV_OFFLOAD_EMERGENCY_PREFILL_RETRY:-1}"
KV_OFFLOAD_EMERGENCY_TRIGGER_RATIO="${KV_OFFLOAD_EMERGENCY_TRIGGER_RATIO:-0.1}"
...
if [[ "${MODE}" == "offload" ]]; then
    CMD+=(
        ...
        --kv-offload-emergency-min-evict-tokens "${KV_OFFLOAD_EMERGENCY_MIN_EVICT_TOKENS}"
        --kv-offload-emergency-prefill-retry "${KV_OFFLOAD_EMERGENCY_PREFILL_RETRY}"
        --kv-offload-emergency-trigger-ratio "${KV_OFFLOAD_EMERGENCY_TRIGGER_RATIO}"
    )
    if [[ "${KV_OFFLOAD_ENABLE_EMERGENCY_EVICTION}" == "1" ]]; then
        CMD+=(--kv-offload-enable-emergency-eviction)
    fi
fi
